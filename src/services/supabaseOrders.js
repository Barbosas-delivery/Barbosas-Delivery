...
export function mapOrderFromDatabase(order, items = []) {
  ...
  return {
    ...
    paymentConfirmedBy: order.payment_confirmed_by || "",
    reopenedAt: order.reopened_at || "",
    reopenedBy: order.reopened_by || "",
    reopenReason: order.reopen_reason || "",
    ...
    couponDiscount: Number(order.coupon_discount || order.discount || 0),
    ...
    items: (Array.isArray(items) ? items : []).map((item) => ({
      ...
      removedIngredients: Array.isArray(item.removed_ingredients) ? item.removed_ingredients : [],
      selectedComboChoices: Array.isArray(item.combo_choices) ? item.combo_choices : Array.isArray(item.customization?.selectedComboChoices) ? item.customization.selectedComboChoices : [],
      itemNote: item.item_note || item.notes || "",
...
export function mapOrderToDatabase(delivery) {
  return {
    ...
    payment_confirmed_at: delivery.paymentConfirmedAt || null,
    payment_confirmed_by: delivery.paymentConfirmedBy || "",
    reopened_at: delivery.reopenedAt || null,
    reopened_by: delivery.reopenedBy || "",
    reopen_reason: delivery.reopenReason || "",
    ...
    coupon_code: delivery.couponCode || "",
    coupon_id: delivery.couponId || null,
    coupon_discount: Number(delivery.couponDiscount || 0),
...
export function mapOrderItemsToDatabase(orderId, items = [], cashSessionId = null) {
  return expandItemsForStock(items).map((item) => ({
    ...
    removed_ingredients: Array.isArray(item.removedIngredients) ? item.removedIngredients : [],
    combo_choices: Array.isArray(item.selectedComboChoices) ? item.selectedComboChoices : [],
    item_note: item.itemNote || item.notes || "",
    customization: {
      ...
      itemNote: item.itemNote || item.notes || "",
      basePrice: Number(item.basePrice || item.price || 0),
      addonsTotal: Number(item.addonsTotal || 0),
      comboChoicesTotal: Number(item.comboChoicesTotal || 0),
    },
