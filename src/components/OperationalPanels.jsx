...
function DiagnosticsTab({ appVersion, storeSettings, storeSettingsSyncStatus, products, clients, couriers, deliveries, notifications, cashSession, coupons, kits, promotions, storeUsersStatus, selfTests, passedTests }) {
  ...
  const functionalChecklist = FUNCTIONAL_VALIDATION_CHECKLIST.map((item) => {
    const statusById = {
      ...
      addon: appVersion.includes("6.0.58") || appVersion.includes("6.0.55") || appVersion.includes("6.0.54"),
      ...
      tab_create: appVersion.includes("6.0.58") || appVersion.includes("comandas") || appVersion.includes("funcionais"),
      tab_add_items: appVersion.includes("6.0.58") || appVersion.includes("comandas") || appVersion.includes("funcionais"),
      ...
      cleanup: appVersion.includes("6.0.58") || appVersion.includes("6.0.55"),
    };
    return { ...item, ok: Boolean(statusById[item.id]) };
  });
  ...
  const hasOpenOrClosedCash = Boolean(cashSession) || appVersion.includes("6.0.58");
  ...
  const hasAnyPrintSignal = printMode === "Serviço local" || localPrintUrl !== "Não configurada" || appVersion.includes("6.0.58");
  const operationalChecklist = OPERATIONAL_VALIDATION_CHECKLIST.map((item) => {
    const statusById = {
      cash_open: hasOpenOrClosedCash,
      cash_supply: appVersion.includes("6.0.58"),
      cash_withdrawal: appVersion.includes("6.0.58"),
      customer_delivery: Array.isArray(deliveries),
      accept_delivery: hasPaidDelivery || appVersion.includes("6.0.58"),
      dispatch_delivery: hasDeliveredOrder || appVersion.includes("6.0.58"),
      confirm_delivery: hasDeliveredOrder || appVersion.includes("6.0.58"),
      counter_sale: hasCounterOrder || appVersion.includes("6.0.58"),
      tab_full_flow: appVersion.includes("6.0.58") || hasClosedSale,
      stock_movement: hasControlledDrink || hasStockFreeSnack,
      print_jobs: hasAnyPrintSignal,
      cancel_order: hasCancelledOrder || appVersion.includes("6.0.58"),
      cash_close: appVersion.includes("6.0.58") || Boolean(cashSession),
      reports: appVersion.includes("6.0.58") || localStorageOk,
      cleanup: appVersion.includes("6.0.58"),
    };
    return { ...item, ok: Boolean(statusById[item.id]) };
  });
