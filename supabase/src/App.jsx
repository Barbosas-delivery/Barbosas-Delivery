...
function App() {
  ...
  async function addProductGroup() {
    ...
    if (error) {
      setStoreSettingsSyncStatus("Grupo criado na tela, mas não foi salvo no Supabase.");
      return setLastAction(`Grupo criado, mas não sincronizado no Supabase: ${error.message || "verifique store_settings."}`);
    }
  ...
  async function saveKitEdits(id) {
    ...
    const { error: deleteItemsError } = await supabase.from("kit_items").delete().eq("kit_id", id);
    if (deleteItemsError) {
      console.error("Erro ao substituir itens do kit:", deleteItemsError);
      return setLastAction(`Kit salvo parcialmente: dados principais salvos, mas itens antigos não foram substituídos (${deleteItemsError.message || "verifique DELETE em kit_items"}).`);
    }
    ...
    const { error: insertItemsError } = await insertWithSchemaRetry("kit_items", itemRows, false);
    if (insertItemsError) {
      console.error("Erro ao salvar itens do kit:", insertItemsError);
      return setLastAction(`Kit salvo parcialmente: dados principais salvos, mas novos itens não foram gravados (${insertItemsError.message || "verifique INSERT em kit_items"}).`);
    }
  ...
  async function addProduct() {
    ...
    if (existingByBarcode.product) {
      ...
      const productPatch = buildProductPatch({ ...newProduct, variants: cleanVariants, active: true });
  ...
  async function applyManualStockAdjustment() {
    ...
    const shouldUseDelta = stockAdjustmentDraft.mode === "entrada" || stockAdjustmentDraft.mode === "saida";
    const stockResult = shouldUseDelta
      ? await applyProductStockDeltasInSupabase([{ productId: product.id, delta: nextStock - currentStock }])
      : await updateWithSchemaRetry("products", product.id, { stock: nextStock });
