...
test("versão da Fase 72 está consistente", () => {
  assert.equal(pkg.version, "6.0.60");
  assert.match(appConstants, /6\.0\.60-fase-72-reteste-operacional-completo/);
  assert.match(electronMain, /6\.0\.60-fase-72-reteste-operacional-completo/);
});
...
test("fila de impressão usa a versão 6.0.60", () => {
  assert.match(printJobs, /templateVersion: "6\.0\.60"/);
  assert.match(printJobs, /template_version: "6\.0\.60"/);
});
