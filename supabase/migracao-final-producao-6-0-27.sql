-- Barbosa's Delivery - Migração final de produção 6.0.27
...
begin
  if p_deltas is null or jsonb_typeof(p_deltas) <> 'array' then
    return jsonb_build_object('success', false, 'failures', jsonb_build_array(jsonb_build_object('error', 'invalid_payload')));
  end if;
  ...
  loop
    ...
    update products
    set stock = greatest(0, coalesce(stock, 0) + delta_rec.delta),
        updated_at = now()
